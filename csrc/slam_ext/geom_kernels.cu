...
__global__ void dense_depth_flow_term_kernel(
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> poses,
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> disps,
    const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> intrinsics,
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> target,
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> weight_in,
    const torch::PackedTensorAccessor32<long, 1, torch::RestrictPtrTraits> ii,
    const torch::PackedTensorAccessor32<long, 1, torch::RestrictPtrTraits> jj,
    const torch::PackedTensorAccessor32<long, 1, torch::RestrictPtrTraits> di,
    torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> residual,
    torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> weight_out,
    torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Ji_out,
    torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Jj_out,
    torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Jz_out,
    torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Jfi_out,
    torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Jfj_out,
    const bool compute_pose_disp,
    ...
    GPU_1D_KERNEL_LOOP(k, pix_count) {
        const int i = k / wd;
        const int j = k % wd;
        const int rbase = 2 * k;
        ...
        const float u = static_cast<float>(j);
        const float v = static_cast<float>(i);
        const float disp = disps[dix][i][j];
        ...
        const float z = Xj[2];
        const float h = Xj[3];
        const float valid = (z > 0.1f && Xi[2] > 0.1f) ? 1.0f : 0.0f;
        const float d = (z > 0.1f) ? 1.0f / z : 1.0f;
        const float d2 = d * d;

        const float proj_u = fx * x * d + cx;
        const float proj_v = fy * y * d + cy;

        residual[block_id][rbase] = proj_u - target[block_id][k][0];
        residual[block_id][rbase + 1] = proj_v - target[block_id][k][1];
        weight_out[block_id][rbase] = valid * weight_in[block_id][k][0];
        weight_out[block_id][rbase + 1] = valid * weight_in[block_id][k][1];
        ...
        Jj_u[2] = fx * (-x * h * d2);
        Jj_u[3] = fx * (-x * y * d2);
        Jj_u[4] = fx * (1.0f + x * x * d2);
        Jj_u[5] = fx * (-y * d);

        Jj_v[0] = 0.0f;
        Jj_v[1] = fy * (h * d);
        Jj_v[2] = fy * (-y * h * d2);
        Jj_v[3] = fy * (-1.0f - y * y * d2);
        Jj_v[4] = fy * (x * y * d2);
        Jj_v[5] = fy * (x * d);

        if (compute_pose_disp) {
            float Ji_u[6];
            float Ji_v[6];
            adjSE3(tij, qij, Jj_u, Ji_u);
            adjSE3(tij, qij, Jj_v, Ji_v);

            for (int n = 0; n < 6; n++) {
                Ji_out[block_id][rbase][n] = -Ji_u[n];
                Ji_out[block_id][rbase + 1][n] = -Ji_v[n];
                Jj_out[block_id][rbase][n] = Jj_u[n];
                Jj_out[block_id][rbase + 1][n] = Jj_v[n];
            }

            Jz_out[block_id][k][0] = fx * (tij[0] * d - tij[2] * x * d2);
            Jz_out[block_id][k][1] = fy * (tij[1] * d - tij[2] * y * d2);
        }
        ...
        if (compute_intrinsics) {
            ...
            Jfi_out[block_id][rbase][0] =
                intrinsics_scale * (fx * d * dXj_df[0] - fx * x * d2 * dXj_df[2]);
            Jfi_out[block_id][rbase + 1][0] =
                intrinsics_scale * (fy * d * dXj_df[1] - fy * y * d2 * dXj_df[2]);
            Jfj_out[block_id][rbase][0] = intrinsics_scale * x * d;
            Jfj_out[block_id][rbase + 1][0] = intrinsics_scale * y * d;
        }
    }
}
...
std::vector<torch::Tensor> dense_depth_flow_term_cuda(torch::Tensor poses, torch::Tensor disps,
    ...
    const int ht = disps.size(1);
    const int wd = disps.size(2);
    const int residual_dim = ht * wd * 2;

    auto opts = disps.options();
    torch::Tensor residual = torch::empty({num, residual_dim}, opts);
    torch::Tensor weight_out = torch::empty({num, residual_dim}, opts);
    torch::Tensor Ji = compute_pose_disp ? torch::empty({num, residual_dim, 6}, opts) : torch::empty({0, 0, 0}, opts);
    torch::Tensor Jj = compute_pose_disp ? torch::empty({num, residual_dim, 6}, opts) : torch::empty({0, 0, 0}, opts);
    torch::Tensor Jz = compute_pose_disp ? torch::empty({num, ht * wd, 2}, opts) : torch::empty({0, 0, 0}, opts);
    torch::Tensor Jfi =
        compute_intrinsics ? torch::empty({num, residual_dim, 1}, opts) : torch::empty({0, 0, 0}, opts);
    torch::Tensor Jfj =
        compute_intrinsics ? torch::empty({num, residual_dim, 1}, opts) : torch::empty({0, 0, 0}, opts);

    dense_depth_flow_term_kernel<<<num, THREADS>>>(
        poses.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        disps.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        intrinsics.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
        ...
        jj.packed_accessor32<long, 1, torch::RestrictPtrTraits>(),
        di.packed_accessor32<long, 1, torch::RestrictPtrTraits>(),
        residual.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        weight_out.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
        Ji.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        Jj.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        Jz.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        Jfi.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
        Jfj.packed_accessor32<float, 3, torch::RestrictPtrTraits>(), compute_pose_disp, compute_intrinsics,
        intrinsics_scale);

    return {residual, weight_out, Ji, Jj, Jz, Jfi, Jfj};
}
