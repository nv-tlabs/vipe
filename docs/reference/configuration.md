...
| `intrinsics_parameterization` | `additive` \| `mei_center_log` | `additive` | - | Local tangent for intrinsics optimization. The experimental MEI-only center-log mode uses η=log(f/(1+ξ)) and β=log(1+ξ), updates fx/fy proportionally, and ignores `distortion_update_scale`. |
