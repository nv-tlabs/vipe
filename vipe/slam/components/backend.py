...
class SLAMBackend:
    ...
    def _iterate_with_depth(self, graph: FactorGraph, steps: int, more_iters: bool):
        ...
        with nvtx_range("slam.backend.iterate_with_depth.update_batch_preintr"):
            graph.update_batch(
        ...
        with nvtx_range("slam.backend.iterate_with_depth.update_disps_sens"):
            self.video.update_disps_sens(self.depth_model, frame_idx=None)
        # Don't update intrinsics again!
        with nvtx_range("slam.backend.iterate_with_depth.update_batch_postintr"):
            graph.update_batch(
                itrs=16 if more_iters else 8,
    ...
    def _iterate_without_depth(self, graph: FactorGraph, steps: int, more_iters: bool):
        with nvtx_range("slam.backend.iterate_without_depth.update_batch"):
            graph.update_batch(
                itrs=16 if more_iters else 8,
                steps=steps,
                optimize_intrinsics=self.args.optimize_intrinsics,
    ...
    def run(self, steps: int = 12, update_depth: bool = True, log: bool = False):
        ...
        with nvtx_range("slam.backend.run"):
            t = self.video.n_frames

            with nvtx_range("slam.backend.run.build_graph"):
                graph = FactorGraph(
                    self.net,
                    self.video,
                    self.device,
            ...
            with nvtx_range("slam.backend.run.add_proximity_factors"):
            ...
            if self.args.adaptive_cross_view:
                with nvtx_range("slam.backend.run.build_adaptive_cross_view_idx"):
